...
frequencia_grupo <- table(dados$Grupo)
percentual_frequencia_grupo <- prop.table(frequencia_grupo) * 100

barplot(frequencia_grupo, main = "Frequência Absoluta por Grupo", xlab =
          "Grupo", ylab = "Frequência Absoluta")
...
estatisticas_notas <- dados %>%
  summarize(
    Média = round(mean(Nota, na.rm = TRUE), 2),
...
tabela_analise_por_grupo <- dados_por_grupo %>%
  summarize(
    ...
    Desvio_Padrão = round(sd(Nota, na.rm = TRUE), 2),
    Curtose = round(kurtosis(Nota, na.rm = TRUE), 2),
    Mínimo = round(min(Nota, na.rm = TRUE), 2),
    Máximo = round(max(Nota, na.rm = TRUE), 2),
    NA_presentes = sum(is.na(Nota))
  )

print(tabela_analise_por_grupo)
...
tabela_notas_e_quartis <- data.frame(
  ...
  `4º Quartil` = c("<= 10,00", porcentagem_filtrada_grupo_xxxx, porcentagem_filtrada_grupo_yyyy, porcentagem_filtrada_grupo_zzzz, "ótimo")
)
